...
//======================= FINANCE CONTROLLER WITH CURRENCY SUPPORT ==============
...
// Helper function to convert INR → target currency
const convertToCurrency = (amountINR, currency) => {
  if (currency === 'INR') return amountINR;
  return amountINR / conversionRates[currency];
};
...
const getFinanceSummary = async (req, res) => {
  try {
    ...
    const totalIncomeINR = incomes.reduce((acc, i) => acc + (i.amount * conversionRates[i.currency]), 0);
    const totalExpenseINR = expenses.reduce((acc, e) => acc + (e.amount * conversionRates[e.currency]), 0);
...
const updateIncome = async (req, res) => {
  try {
    ...
    const updated = await Income.findByIdAndUpdate(
      id,
      { title, type, amount, currency, date, category, notes },
      { new: true }
    );
    ...
    res.json({ success: true, message: "Income updated", updated });
...
const updateExpense = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Expense updated", updated });
  ...
  try {
    ...
    incomes.forEach((i) =>
      sheet.addRow({
        ...
        amount: i.amount,
        currency: i.currency,
        amountINR: i.amount * conversionRates[i.currency],
        date: new Date(i.date).toLocaleDateString("en-IN"),
        notes: i.notes || "-",
      })
    );
    ...
    expenses.forEach((e) =>
      sheet.addRow({
        ...
        amount: e.amount,
        currency: e.currency,
        amountINR: e.amount * conversionRates[e.currency],
        date: new Date(e.date).toLocaleDateString("en-IN"),
        notes: e.notes || "-",
      })
    );
    ...
    // Summary Section  
    sheet.addRow([]);
    sheet.addRow(["SUMMARY (" + currency + ")", "", "", "", "", ""]);
    sheet.addRow(["Total Income", totalIncome]);
    sheet.addRow(["Total Expense", totalExpense]);
    sheet.addRow(["Balance", balance]);

    sheet.addRow([]);
    sheet.addRow(["SUMMARY (INR)", "", "", "", "", ""]);
    sheet.addRow(["Total Income (INR)", totalIncomeINR]);
    sheet.addRow(["Total Expense (INR)", totalExpenseINR]);
    sheet.addRow(["Balance (INR)", balanceINR]);

    // File name
    const filename = `finance_report_${currency}.xlsx`;
